...
CREATE TABLE IF NOT EXISTS `sakila_snowflake`.`fact_rental` (
    ...
    `film_key` INT(11) NOT NULL,
    `store_key` INT(8) NOT NULL,
    `rental_date_key` BIGINT(20) NOT NULL,
    `return_date_key` BIGINT(20) NULL,
    `count_rentals` INT(11) NOT NULL,
    `count_returns` INT(8) NOT NULL,
    `rental_duration` INT(10) NOT NULL,
    `dollar_amount` FLOAT NOT NULL,
    PRIMARY KEY (`rental_id`),
    INDEX `fk_fact_rental_dim_customer_idx` (`customer_key` ASC),
    INDEX `fk_fact_rental_dim_staff_idx` (`staff_key` ASC),
    INDEX `fk_fact_rental_dim_film_idx` (`film_key` ASC),
    INDEX `fk_fact_rental_dim_store_idx` (`store_key` ASC),
    INDEX `fk_fact_rental_dim_date_idx1` (`rental_date_key` ASC),
    INDEX `fk_fact_rental_dim_date_idx2` (`return_date_key` ASC),
    ...
    CONSTRAINT `fk_fact_rental_dim_staff` FOREIGN KEY (`staff_key`)
        ...
        ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT `fk_fact_rental_dim_film` FOREIGN KEY (`film_key`)
        REFERENCES `sakila_snowflake`.`dim_film` (`film_key`)
        ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT `fk_fact_rental_dim_store` FOREIGN KEY (`store_key`)
        REFERENCES `sakila_snowflake`.`dim_store` (`store_key`)
        ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT `fk_fact_rental_dim_date1` FOREIGN KEY (`rental_date_key`)
        REFERENCES `sakila_snowflake`.`dim_date` (`date_key`)
        ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT `fk_fact_rental_dim_date2` FOREIGN KEY (`return_date_key`)
        REFERENCES `sakila_snowflake`.`dim_date` (`date_key`)
        ON DELETE NO ACTION ON UPDATE NO ACTION
