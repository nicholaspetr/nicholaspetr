...
(rental_id, rental_last_update, customer_key, staff_key,
film_key, store_key, rental_date_key, 
...
df.film_key,
dstr.store_key,
d_rental.date_key as rental_date_key, 
d_return.date_key as return_date_key, 
CASE WHEN r.rental_date IS NULL THEN 0 ELSE 1 END AS count_rentals,
CASE WHEN r.return_date IS NULL THEN 0 ELSE 1 END AS count_returns,
f.rental_duration as rental_duration,
f.rental_rate as dollar_amount
...
INNER JOIN sakila.store str ON i.store_id=str.store_id
INNER JOIN sakila.customer c ON r.customer_id=c.customer_id
INNER JOIN dim_date d_rental
ON DATE(r.rental_date) = d_rental.date
LEFT JOIN dim_date d_return
ON DATE(r.return_date) = d_return.date
inner join sakila_snowflake.dim_film df 
on f.film_id = df.film_id
inner join sakila_snowflake.dim_customer dc
on c.customer_id = dc.customer_id
inner join sakila_snowflake.dim_staff ds on 
s.staff_id = ds.staff_id
inner join sakila_snowflake.dim_store dstr on
str.store_id = dstr.store_id;
